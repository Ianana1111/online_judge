...
- Canonical editorial inventory: 430 problems and 430 bilingual teaching texts. Current draft coverage is C++17 394, Python 3 39, C11 3, Java 17 3. **3/430 have all four languages**; 1,281 additional programs and matching Chinese/English code explanations remain.
...
- Three new four-language drafts (`uva-10071-back-to-high-school-physics`, `uva-10055-hashmat`, `uva-11150-cola`) passed their saved sample/hidden cases locally. All 12 language variants also passed full sample/hidden execution and aggregate Submit evaluation in the Vercel Judge Sandbox; their existing incorrect C++ mutations were rejected. Private reports are under `generated/editorial-audit-20260925/`. The audit had a Java-only false CE on its second compile into a frozen directory; `scripts/editorials/audit.ts` now performs Java's aggregate Submit check in a fresh sandbox, matching a real submission.
- Docker's local daemon still does not answer, so the required Docker-side evidence, release review, publication, and deployment for these drafts are pending. The production site does not yet show the three new language sets or changed C++ headers.
