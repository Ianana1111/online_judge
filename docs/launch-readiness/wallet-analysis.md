...
查證日期：2026-09-15。現況是綠界信用卡月／年自動續訂。此文件不代表 Apple Pay 已啟用，也不把延遲付款視為定期扣款。

| 方案 | 與目前產品的關係 | 實作及商家依賴 |
| --- | --- | --- |
| 綠界信用卡定期定額 | 維持目前續訂、取消、退款流程 | 完成商家控制的實卡首扣、續扣、取消與退款驗收 |
| 綠界 Apple Pay 單次購買 | 可另設不續訂的固定期間方案；不能直接代替現有訂閱欄位 | 後台申請資格、依所選介接確認網域驗證、Apple Pay 裝置驗收、一次性訂單與權益處理 |
...
App Store 的數位功能／訂閱需檢查應用內購買要求、跨平台存取與地區例外；Apple Pay 並不直接取代 StoreKit。這是未來原生 App 的上架設計事項，目前網站發布不需要先新增 StoreKit。[Apple 審查規範 3.1](https://developer.apple.com/app-store/review/guidelines/)、[Apple 自動續訂訂閱](https://developer.apple.com/app-store/subscriptions/)。

建議：先保留既有信用卡自動續訂，完成真實首扣／續扣驗收。若要先增加 Apple Pay，明確新增「單次購買、不自動續訂」商品，維持原訂閱商品的價格與條款。此為技術建議，尚未改變商品或收費。
...
站長可依[正式驗收清單](owner-acceptance.md)操作。最新[綠界會員服務說明](https://support.ecpay.com.tw/25120/)列 Apple Pay 為各會員類型可用，但[站內付介接頁](https://developers.ecpay.com.tw/57359/)仍列特約流程；商家資格需針對所選方案核對。跳轉付款與站內付的網域驗證要求不可混用。[綠界 Web 版文件](https://developers.ecpay.com.tw/9040/)說明該方案不需自行申請 Apple Developer 帳號，並不涵蓋原生 App 上架。
