...
export default defineConfig({
  ...
  projects: [{ name: "desktop", use: { ...devices["Desktop Chrome"], viewport: { width: 1440, height: 1000 } } },
    ...
    ...(process.env.CROSS_BROWSER_E2E === "1" ? [
      { name: "firefox", use: { ...devices["Desktop Firefox"] }, testMatch: /(?:account-security|public-pages|schools|community|notifications|refunds|refund-resolution|pricing|collections|run-consistency)\.spec\.ts/ },
      { name: "webkit", use: { ...devices["Desktop Safari"] }, testMatch: /(?:account-security|public-pages|schools|community|notifications|refunds|refund-resolution|pricing|collections|run-consistency)\.spec\.ts/ },
      { name: "ios-webkit", use: { ...devices["iPhone 13"] }, testMatch: /(?:account-security|public-pages|schools|community|notifications|refunds|refund-resolution|pricing|collections|run-consistency)\.spec\.ts/ },
    ] : []),
  ],
