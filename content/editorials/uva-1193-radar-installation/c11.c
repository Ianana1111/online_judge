...
int main(void){char *token;int tc=0;while((token=word())!=NULL){int n=atoi(token);free(token);char *radius=signed_normalize(word());if(!n&&strcmp(radius,"0")==0){free(radius);break;}char *rr=signed_multiply(radius,radius);Interval *intervals=(Interval*)malloc((n+1)*sizeof(Interval));int possible=sign(radius)>=0;
    for(int i=0;i<n;i++){char *x=signed_normalize(word()),*y=signed_normalize(word()),*yy=signed_multiply(y,y),*square=signed_subtract(rr,yy);if(sign(square)<0){possible=0;free(square);square=copy("0");}intervals[i]=(Interval){endpoint(x,-1,square),endpoint(x,1,square)};free(y);free(yy);}
    int answer=-1;if(possible){qsort(intervals,n,sizeof(Interval),right_compare);answer=0;char zero[]="0";Endpoint position=endpoint(zero,1,zero);for(int i=0;i<n;i++)if(!answer||endpoint_compare(intervals[i].left,position)>0){position=intervals[i].right;answer++;}}
    printf("Case %d: %d\n",++tc,answer);for(int i=0;i<n;i++){free(intervals[i].left.center);free(intervals[i].left.square);}free(intervals);free(radius);free(rr);
}return 0;}
