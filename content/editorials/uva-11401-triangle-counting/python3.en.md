...
Storing one million Python integers exceeds this problem’s memory limit. Sort distinct query bounds, extend the running total only as far as each bound, and store answers only for requested bounds. Finally print them in the original query order.
