...
Java derives a safe bound from the largest individual amount, simple-road path length, and all offers. When that bound guarantees long arithmetic is exact, it runs the same algorithm with primitive arrays to avoid allocating objects per transition. Otherwise it retains BigInteger, without truncating large amounts.
