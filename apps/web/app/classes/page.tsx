...
export default function MyClassesPage() {
  ...
  if (status === "ready" && !user) {
    return <p className="text-sm text-verdict-wa">{t("Log in to see your classes.")}</p>;
  }

  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl font-bold text-ink-50">{t("My Classes")}</h1>
        <p className="mt-1 text-sm text-ink-400">
          {t("What was taught each session, and the homework that came with it. Open a class to see the full content and ask questions.")}
        </p>
      </div>

      {isLoading && <SkeletonList rows={4} />}
      {classes?.length === 0 && (
        <p className="oj-card p-4 text-sm text-ink-400">{t("No classes have been recorded for you yet.")}</p>
      )}
