...
export default function DiscussionListClient() {
  ...
  return <div className="mx-auto max-w-6xl space-y-8">
    <header className="relative overflow-hidden rounded-2xl border border-ink-700 bg-ink-900 p-6 sm:p-9">
      ...
      <div className="relative mt-6 flex flex-wrap gap-3"><Link href={user ? "/discussion/write" : "/login?next=/discussion/write"} className="oj-btn-primary">{zh ? "＋ 發起討論" : "+ Start a discussion"}</Link>{user && <Link href="/discussion/mine" className="oj-btn-ghost">{zh ? "我的投稿" : "My posts"}</Link>}</div>
    </header>
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_240px]">
      <section className="min-w-0" aria-label={zh ? "社群文章" : "Community posts"}>
        <form onSubmit={(e) => { e.preventDefault(); setSearch(input.trim()); }} className="mb-5 flex gap-2"><label htmlFor="community-search" className="sr-only">{zh ? "搜尋討論" : "Search discussions"}</label><input id="community-search" className="oj-input min-w-0 flex-1" type="search" maxLength={100} value={input} onChange={(e) => setInput(e.target.value)} placeholder={zh ? "搜尋題目、觀念或關鍵字" : "Search titles, concepts or keywords"} /><button className="oj-btn-ghost" type="submit">{zh ? "搜尋" : "Search"}</button></form>
        ...
        {query.isError && <div role="alert" className="oj-card p-6"><p>{zh ? "暫時無法載入討論。" : "Could not load discussions."}</p><button className="oj-btn-ghost mt-3" onClick={() => query.refetch()}>{zh ? "重新載入" : "Retry"}</button></div>}
        ...
        {query.hasNextPage && <button className="oj-btn-ghost mt-5 w-full" disabled={query.isFetchingNextPage} onClick={() => query.fetchNextPage()}>{zh ? "載入更多討論" : "Load more discussions"}</button>}
      </section>
