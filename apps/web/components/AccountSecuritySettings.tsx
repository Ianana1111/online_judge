  ...
  return <div className="space-y-6">
    ...
    <section className="oj-card space-y-4 p-5" aria-labelledby="mfa-security-title"><h2 id="mfa-security-title" className="font-semibold text-ink-100">{zh ? "雙因素驗證" : "Two-factor authentication"}</h2>
      ...
      {codes.length > 0 ? <div className="space-y-4"><h3 className="text-sm font-semibold text-ink-100">{zh ? "保存備用碼" : "Save recovery codes"}</h3><p className="text-sm leading-6 text-ink-300">{zh ? "每組只能使用一次，之後無法再次查看。請存放在安全的地方，避免與驗證器放在同一部裝置。" : "Each code works once and cannot be displayed again. Store them safely, separate from your authenticator."}</p><pre className="overflow-x-auto rounded-lg bg-ink-900 p-4 text-sm text-ink-100" tabIndex={0} aria-label={zh ? "備用碼清單" : "Recovery codes"}>{codes.join("\n")}</pre><button className="oj-btn-primary" onClick={() => setCodes([])}>{zh ? "我已安全保存備用碼" : "I have saved my recovery codes"}</button></div> : <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); void action(setup ? "enable" : user.mfaEnabled ? "recovery-codes" : "setup"); }}>
        ...
        {setup && <div className="space-y-4 rounded-xl border border-ink-700 bg-ink-900/40 p-4 sm:p-5">
          <div><h3 className="font-semibold text-ink-100">{zh ? "掃描 QR Code 加入驗證器" : "Scan to add your authenticator"}</h3><p className="mt-1 text-sm leading-6 text-ink-300">{zh ? "開啟 Google Authenticator，點選「＋」並選擇「掃描 QR code」。加入後，請在下方輸入顯示的六位數驗證碼。" : "In Google Authenticator, tap + and choose Scan a QR code. Then enter the six-digit code below to finish setup."}</p></div>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
            <div className="w-fit rounded-xl bg-white p-2"><QRCodeSVG value={setup.uri} size={208} marginSize={4} level="M" bgColor="#FFFFFF" fgColor="#111827" role="img" title={zh ? "judge.tw 雙因素驗證設定 QR code" : "judge.tw authenticator setup QR code"} /></div>
            <div className="space-y-2 text-sm leading-6 text-ink-300"><p>{zh ? "在同一支手機上設定？可直接開啟驗證器。" : "Setting up on this phone? Open your authenticator directly."}</p><a href={setup.uri} className="inline-block text-brand underline">{zh ? "在此裝置開啟驗證器" : "Open authenticator on this device"}</a></div>
          </div>
          <details className="rounded-lg border border-ink-700 p-3 text-sm"><summary className="cursor-pointer font-medium text-ink-200">{zh ? "無法掃描？改用手動金鑰" : "Can't scan? Enter the setup key manually"}</summary><div className="mt-3 break-all rounded-lg bg-ink-800 p-3 font-mono text-ink-100" aria-label={zh ? "驗證器設定金鑰" : "Authenticator setup key"}>{setup.secret}</div><p className="mt-2 leading-6 text-ink-300">{zh ? "帳號名稱填 judge.tw，金鑰類型選「依時間」。" : "Use judge.tw as the account name and select a time-based key."}</p></details>
          <p className="text-xs leading-5 text-ink-400">{zh ? "設定須於十分鐘內完成。QR code 與金鑰請勿分享。" : "Finish within ten minutes. Do not share this QR code or setup key."}</p>
        </div>}
