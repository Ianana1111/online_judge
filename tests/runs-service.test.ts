...
it("rejects a stale expected answer even when the input is unchanged", async () => {
  await expect(new RunsService(queue as any, redis as any).create("owner", { ...dto, cases: [{ ...dto.cases[0], sampleRevision: await sampleRevision("1\n", "outdated answer") }] })).rejects.toThrow("Sample has changed");
