...
describe.skipIf(process.env.RUN_DB_TESTS!=="1")("official editorial publication and data-version gates",()=>{
  ...
  beforeAll(()=>{const u=new URL(process.env.DATABASE_URL??"invalid:");if(u.hostname!=="127.0.0.1"||u.port!=="55432"||u.pathname!=="/oj_test")throw new Error("Disposable database required");});
  ...
  async function user(){const u=await prisma.user.create({data:{handle:`ed_${randomUUID()}`,email:`${randomUUID()}@example.test`}});users.push(u.id);return u;}
  ...
  it("keeps drafts unavailable, exposes only valid published content and never returns private evidence",async()=>{
    const p=await problem();expect(await service.detail(p.slug,null)).toEqual({status:"NOT_READY"});
    const draft=await publish(p.id,{publishedAt:null});expect(await service.detail(p.slug,null)).toEqual({status:"NOT_READY"});
    await prisma.problemEditorial.update({where:{id:draft.id},data:{publishedAt:new Date()}});
    const result=await service.detail(p.slug,null);expect(result.status).toBe("AVAILABLE");expect(JSON.stringify(result)).not.toContain("must-not-leak");expect(JSON.stringify(result)).not.toContain("judgeFingerprint");
    await prisma.problem.update({where:{id:p.id},data:{visibility:false}});await expect(service.detail(p.slug,null)).rejects.toThrow("Problem not found");
  });
  ...
  it("invalidates on case insert, answer update and deletion, but keeps no-op writes valid",async()=>{
    ...
    expect(await service.detail(p.slug,null)).toEqual({status:"REVIEW_REQUIRED"});await publish(p.id);
    await prisma.testCase.update({where:{id:tc.id},data:{output:"1\n"}});expect((await service.detail(p.slug,null)).status).toBe("AVAILABLE");
    await prisma.testCase.update({where:{id:tc.id},data:{output:"2\n"}});expect(await service.detail(p.slug,null)).toEqual({status:"REVIEW_REQUIRED"});await publish(p.id);
    await prisma.testCase.delete({where:{id:tc.id}});expect(await service.detail(p.slug,null)).toEqual({status:"REVIEW_REQUIRED"});
  });
  it("invalidates sample edits and changes to the statement, limits or checker",async()=>{
    ...
    expect((await service.detail(p.slug,null)).status).toBe("REVIEW_REQUIRED");await publish(p.id);
    await prisma.sample.update({where:{id:sample.id},data:{output:"2"}});expect((await service.detail(p.slug,null)).status).toBe("REVIEW_REQUIRED");
    for(const data of [{statementMd:"New semantics"},{timeLimitMs:1234},{memoryLimitKb:32768},{checkerType:"EXACT" as const}]) {await publish(p.id);await prisma.problem.update({where:{id:p.id},data});expect((await service.detail(p.slug,null)).status).toBe("REVIEW_REQUIRED");}
  });
  it("invalidates both problems when a sample is moved",async()=>{
    ...
    await prisma.sample.update({where:{id:s.id},data:{problemId:b.id}});expect((await service.detail(a.slug,null)).status).toBe("REVIEW_REQUIRED");expect((await service.detail(b.slug,null)).status).toBe("REVIEW_REQUIRED");
  });
  it("preserves the old published snapshot and requires a new revision for content edits",async()=>{
    ...
    const old=await prisma.problemEditorial.findUniqueOrThrow({where:{id:first.id}});expect(old.content).toEqual(first.content);expect(old.supersededAt).not.toBeNull();expect((await service.detail(p.slug,null))).toMatchObject({status:"AVAILABLE",revision:2});
  });
  it("fails closed for an old judge implementation or malformed publication",async()=>{
    const p=await problem();await publish(p.id,{judgeRevision:"outdated-pipeline"});expect(await service.detail(p.slug,null)).toEqual({status:"REVIEW_REQUIRED"});
    await publish(p.id,{content:{invalid:true}});expect(await service.detail(p.slug,null)).toEqual({status:"REVIEW_REQUIRED"});
  });
  ...
  it("previews without writes, applies corpus+publication atomically, and is idempotent",async()=>{
    ...
    expect(await service.detail(plan.slug,null)).toEqual({status:"NOT_READY"});
    expect((await applyPublications([plan],true))[0]).toMatchObject({status:"PUBLISHED",revision:1});
    expect((await service.detail(plan.slug,null)).status).toBe("AVAILABLE");
